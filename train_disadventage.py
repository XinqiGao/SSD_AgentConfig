...
def build_experiment_config_dict(args):
    ...
    def gen_policy_disadvantageous():
        return None, obs_space, act_space, {
            "custom_model": "PPO",
            "inequity_averse_reward": True,
            "alpha": 5.0,  # Disadvantageous inequity aversion
            "beta": 0.0,  # No Advantageous inequity aversion
        }

    def gen_policy_normal():
        return None, obs_space, act_space, {
            "custom_model": model_name,
            "inequity_averse_reward": True,
            "alpha": 0.0,  # No inequity aversion
            "beta": 0.0
        }

    # For heterogeous Population: Create heterogeneous policy mapping
    policy_graphs = {}
    for i in range(args.num_agents):
        if i < 3:  # First 3 agents are disadvantageous inequity-averse
            policy_graphs["agent-" + str(i)] = gen_policy_disadvantageous()
        else:  # Last 2 agents are normal PPO
            policy_graphs["agent-" + str(i)] = gen_policy_normal()


    # Create 1 distinct policy per agent:3 Disadvantageous-Inequity-Averse Agents and 2 PPO Agents
    # policy_graphs = {}
    # for i in range(args.num_agents):
    #     policy_graphs["agent-" + str(i)] = gen_policy()

    #For heterogeous Population:
    # policy_graphs = {}
    # for i in range(args.num_agents):
    #     if i < 3:  # First 3 agents are disadvantageous inequity-averse
    #         policy_graphs["agent-" + str(i)] = gen_policy_disadvantageous()
    #     else:  # Last 2 agents are standard PPO
    #         policy_graphs["agent-" + str(i)] = gen_policy()
